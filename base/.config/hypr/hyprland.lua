...
hl.on("hyprland.start", function()
    ...
    hl.exec_cmd("[workspace 3 silent] zeditor")
    hl.exec_cmd("[workspace 4 silent] google-chrome")
    hl.exec_cmd("[workspace 5 silent] slack")
...
hl.config({
    general = {
        ...
        col = {
            active_border   = { colors = { c.mauve, c.pink }, angle = 45 },
...
hl.bind("ALT + TAB",         hl.dsp.exec_cmd("~/bin/stew win"))
...
-- workspace switch + move-to-workspace (1..10, where 10 is the "0" key)
for i = 1, 10 do
    local key = i % 10
...
-- move/resize with Super + LMB/RMB
hl.bind(mainMod .. " + mouse:272", hl.dsp.window.drag(),   { mouse = true })
hl.bind(mainMod .. " + mouse:273", hl.dsp.window.resize(), { mouse = true })

-- multimedia / brightness (locked = works on lockscreen; repeating = key-repeat)
...
hl.window_rule({
    name  = "zoom-toolbar",
    match = { class = "^(zoom|Zoom|zoom-us)$", title = "^(as_toolbar.*)$" },
    pin      = true,
    no_focus = true,
})

-- steam — float child windows, keep main client tiled
hl.window_rule({
    name  = "steam-float-children",
    match = { class = "^(steam)$" },
    float = true,
})
hl.window_rule({
    name  = "steam-main-tiles",
